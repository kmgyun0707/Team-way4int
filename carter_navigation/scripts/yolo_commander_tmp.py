...
class YoloCommander(Node):
    def __init__(self):
        ...
        self.bag_detection_threshold = 1 # 즉각 반응
        self.person_max_detection_range = 3.0 

        self.get_logger().info(">>> YOLO Commander Ready (Simple Left Move Strategy)")
    ...
    # [핵심 기능] 왼쪽으로 2m 이동하는 좌표 계산 함수
    def get_left_goal_pose(self):
        try:
            # 1. 로봇(base_link) 기준으로 왼쪽 2m 좌표 생성
            left_pose = PoseStamped()
            left_pose.header.frame_id = 'base_link'
            left_pose.header.stamp = self.get_clock().now().to_msg()
            
            left_pose.pose.position.x = 0.5  # 앞으로 살짝 (0.5m)
            left_pose.pose.position.y = 2.0  # 왼쪽으로 많이 (2.0m)
            left_pose.pose.position.z = 0.0
            
            # 2. 로봇 헤딩도 왼쪽(90도)으로 돌려줌
            # Quaternion (0, 0, 0.707, 0.707) = 90도 회전
            left_pose.pose.orientation.z = 0.707
            left_pose.pose.orientation.w = 0.707
            
            # 3. Map 좌표계로 변환 (Nav2에게 줄 때 필요)
            target_pose = self.tf_buffer.transform(
                left_pose, 
                'map', 
    ...
    def rgb_callback(self, msg):
        ...
        results = self.model(frame, classes=[0, 24, 26, 28], verbose=False, conf=0.25)
        ...
        if final_target and final_target['dist'] > 0:
            ...
            # [CASE 1: 가방] 발견 즉시 왼쪽으로 2m 도망가기
            if target_type == "BAG":
                if self.mode == "NAVIGATING":
                    if self.bag_detection_count >= self.bag_detection_threshold:
                        self.get_logger().warn(f">>> 🎒 BAG Detected! GO LEFT 2.0m!")
                        self.stop_robot() # 일단 정지
                        
                        # [핵심] 가방 위치 계산 안 함. 그냥 내 기준 왼쪽으로 쏨.
                        left_goal = self.get_left_goal_pose()
                        
                        if left_goal:
                            self.mode = "MOVING_LEFT" # 상태 변경 (중복 명령 방지)
                            self.send_nav_goal(left_goal, callback_type="LEFT")
                        else:
                            self.mode = "NAVIGATING" # 실패 시 원복
            ...
            # [CASE 2: 사람]
                ...
                if self.mode != "IDLE" and self.mode != "MOVING_LEFT":
                    if dist <= self.person_max_detection_range:
                        if dist > 0.3: 
                            target_pose = self.calculate_map_pose(x1, y1, x2, y2, dist, msg.header) # Args fixed order
                            # Note: calculate_map_pose arguments in previous snippets were (x1, x2, y1, y2...)
                            # Let's keep consistency with prev working code.
                            # Re-checking arguments: calculate_map_pose(self, x1, x2, y1, y2, dist, header_rgb)
                            x1, y1, x2, y2 = final_target['box']
