...
# 이동 경로 시각화 기능 추가
# 목표지점까지 재 이동은 0.35m 앞에서 멈추도록 수정- 이게 제일 잘됨
class YoloCommander(Node):
    def __init__(self):
        ...
        # [NEW] 이동 경로(Path) 시각화를 위한 퍼블리셔 추가
        self.pub_path = self.create_publisher(Path, '/robot_path', 10)
        self.history_path = Path()
        self.history_path.header.frame_id = 'map' # 지도 기준 경로
        # 0.2초마다 경로 저장 (너무 빠르면 데이터 폭증하므로 적당히 조절)
        ...
        self.get_logger().info(">>> YOLO Commander Ready (Path Visualization Enabled)")

    def update_path_callback(self):
        """로봇의 현재 위치를 지속적으로 기록하여 Path로 발행하는 함수"""
        try:
            # map 기준 base_link(로봇 몸체) 위치 조회
            transform = self.tf_buffer.lookup_transform(
                'map', 
                'base_link', 
                rclpy.time.Time(seconds=0) # 최신 위치
            )
            ...
            # 너무 길어지면 앞부분 삭제 (선택사항: 메모리 관리용, 필요 없으면 주석 처리)
            if len(self.history_path.poses) > 5000:
                self.history_path.poses.pop(0)

            # Path 토픽 발행 -> RViz에서 이걸 구독하면 선이 그려짐!
            self.pub_path.publish(self.history_path)

        except (LookupException, ConnectivityException, ExtrapolationException):
            pass # 아직 TF가 준비 안 됐으면 무시
        ...
        if best_box:
            ...
            if dist > 0:
                if self.mode == "NAVIGATING":
                    self.get_logger().warn(f">>> Person Detected! Mode: FOLLOW")
                    self.stop_robot()
                    self.mode = "FOLLOWING"
                    return 

                if self.mode == "FOLLOWING":
                    if dist > 1.2: # 사람이 1.2m 보다 멀리있는지 확인 정지 기준선 
                ...
                cv2.putText(frame, f"{dist:.1f}m", (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
        else:
            if self.mode == "FOLLOWING":
                self.stop_robot()
                self.mode = "IDLE"
